...
import { ArrowRight, FileText, MousePointer, CheckCircle, Palette, Heart, Download } from 'lucide-react';
...
const LandingPage = () => {
  ...
  useEffect(() => {
    ...
    cards?.forEach((card) => featureObserver.observe(card));
  ...
  return (
    <>
      ...
      {/* Hero Section */}
      <section ref={heroRef} className="pt-24 pb-16 bg-gradient-to-br from-[#F8F9FA] to-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className={`transition-all duration-800 ${isHeroVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10'}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  to="/register"
                  className="bg-[#2F3C7E] text-white px-8 py-4 rounded-lg hover:bg-[#00C9A7] transition-all duration-200 font-semibold flex items-center justify-center group"
                >
                  Get Started
                ...
                <a
                  href="#features"
                  className="border-2 border-[#2F3C7E] text-[#2F3C7E] px-8 py-4 rounded-lg hover:bg-[#2F3C7E] hover:text-white transition-all duration-200 font-semibold flex items-center justify-center"
                >
            ...
            {/* Right Side */}
            <div className={`relative transition-all duration-800 delay-200 ${isHeroVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-xl text-[#6C757D] max-w-3xl mx-auto">
              Everything you need to craft, design, and export the perfect resume — no complexity, no charges.
            </p>
          </div>
            ...
            {features.map((feature, index) => {
              ...
              return (
                <div
                  key={index}
                  data-index={index}
                  className={`feature-card bg-[#F8F9FA] p-8 rounded-2xl hover:shadow-lg transition-all duration-500 hover:-translate-y-2 relative ${
                ...
                >
                  {feature.badge && (
                    <div className="absolute -top-3 -right-3">
                      <span className="bg-[#00C9A7] text-white px-3 py-1 rounded-full text-xs font-medium">{feature.badge}</span>
                    </div>
                  )}
